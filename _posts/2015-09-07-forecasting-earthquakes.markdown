...
Because we'll never *truly* have the [initial state of anything,][hsp] perfectly accurate predictions of phenomena effected by quantum effects (like partical physics) is next to impossible. But for larger scale phenomena like the weather or **Earthquakes**, we've gotten really good at creating accurate, reliable forecasts. The major difference between a forecast and a prediction is a prediction states that a particular event will happen at time X assuming Y conditions are met, whereas a forecast just gives a probability of such an event taking place.
 


I've thrown my hat into the fray of near term earthquake forecasting, and over the next week or so I'll be discussing different topics that are important when trying to tackle such a challenging problem.
